; $ID:	GOM_BLOOM_2023_MAIN.PRO,	2024-01-30-21,	USER-KJWH	$
  PRO GOM_BLOOM_2023_MAIN, VERSION, LOGFILE=LOGFILE, LOGLUN=LOGLUN, BUFFER=BUFFER, OVEWRITE=OVERWRITE, _EXTRA=EXTRA
...
; ****************************************************************************************************
  ...
  FOR V=0, N_ELEMENTS(VERSION)-1 DO BEGIN
    ...
    ;  PROJECT_MAKE_COMPOSITE, VERSTR, MAPP='MASS_BAY',PRODS='CHLOR_A',/WEEKS,/ADD_COLORBAR,/ADD_BATHY,/ADD_BOARDER,OUT_COLOR=0,BUFFER=1,DIR_OUT=VERSTR.DIRS.DIR_COMPOSITES+'MASS_BAY'+SL
      PROJECT_MAKE_COMPOSITE, VERSTR, PRODS='CHLOR_A',/WEEKS,/ADD_LONLAT,LONS=[-73,-70,-67],LATS=[40,42,44],/ADD_COLORBAR,/ADD_BATHY,/ADD_BORDER,OUT_COLOR=0,BUFFER=1
    ENDIF

    IF KEYWORD_SET(ANIMATIONS) THEN $
      PROJECT_COMPOSITE_ANIMATION, VERSTR, /WEEKS, EXTENSIONS=['webm'],BUFFER=BUFFER, FRAMES=2, /ADD_COLORBAR, /ADD_BATHY, /ADD_LONLAT, LONS=[-73,-70,-67],LATS=[40,42,44],/ADD_BORDER;, OUTCOLOR=0
      ...
      FOR M=0, N_ELEMENTS(MTHS)-1 DO BEGIN
        ...
        FOR Y=0, N_ELEMENTS(YEARS)-1 DO BEGIN
         ...
         ;   IF PR.IN_PROD NE PR.PROD THEN PROD_SCALE = PR.IN_PROD ELSE PROD_SCALE = [];
            ...
            IMG = PROJECT_MAKE_IMAGE(VERSTR, FILE=FILE, DATERANGE=DR,BUFFER=BUFFER, RESIZE=RESZ, MAPP=MP, PROD_SCALE=PROD_SCALE,$
              /ADD_LONLAT,LONS=[-73,-70,-67],LATS=[40,42,44],/ADD_COLORBAR,/ADD_BATHY,/ADD_BORDER,OUT_COLOR=0, _EXTRA=EXTRA)
    ...
    IF LUN NE [] THEN BEGIN & FLUSH, LUN & CLOSE, LUN & FREE_LUN, LUN & END
